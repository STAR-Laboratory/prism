...
// Define to enable the ABO-bookkeeping counters below
// #define PRISM_DEBUG_STATS
...
class PRISMDRAMController final : public IBHDRAMController, public Implementation {
    RAMULATOR_REGISTER_IMPLEMENTATION(IBHDRAMController, PRISMDRAMController, "PRISMDRAMController", "PrISM DRAM controller.")
...
private:
    ...
    ReqBuffer m_alert_buffer;             // PrISM ABO recovery (PREall + RFMab).
    ...
    IPRISM::ABOState  m_cur_prism_state       = IPRISM::ABOState::NORMAL;
...
#ifdef PRISM_DEBUG_STATS
    // ---- ABO bookkeeping counters (debug / paper data) ---------------------
    uint64_t s_num_cycles_alert_buffer_nonempty = 0;
    uint64_t s_num_cycles_alert_buffer_nonempty_before_recovery = 0;
    uint64_t s_num_demand_rejected_due_to_fits = 0;
    uint64_t s_num_active_rejected_due_to_fits = 0;
    uint64_t s_num_demand_deferred_rank_blocked = 0;     // alert-buffer defer
    uint64_t s_num_demand_deferred_pending_maint = 0;    // priority-buffer maintenance defer

    uint64_t s_num_cycles_alert_buffer_head_prea = 0;
    uint64_t s_num_cycles_alert_buffer_head_rfmab = 0;
    uint64_t s_num_cycles_alert_buffer_head_not_ready = 0;
    uint64_t s_num_demand_issued_while_alert_buffer_nonempty = 0;
    uint64_t s_num_active_issued_while_alert_buffer_nonempty = 0;
    uint64_t s_num_cycles_alert_buffer_head_other = 0;
    uint64_t s_num_cycles_alert_buffer_rfmab_blocked_by_pre_recovery = 0;
    uint64_t s_num_priority_issued_while_alert_buffer_nonempty = 0;

    // [PrISM] classifies each PREall we enqueue.
    //   necessary   = banks were open, precharge did real work
    //   unnecessary = banks already closed; PREall was wasted but harmless
    //                 (it overlaps tABO_ACT, so no recovery-window cost)
    uint64_t s_num_prea_necessary   = 0;
    uint64_t s_num_prea_unnecessary = 0;
#endif
...
public:
    void init() override {
        ...
        m_prism = get_plugin<IPRISM>();
        if (!m_prism) {
            std::cout << "[PRISMCTRL] Need PRISM plugin!";
            std::exit(0);
        }
    };
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
        ...
        m_alert_buffer.max_size    = num_ranks * (1 + m_prism->get_num_abo_recovery_refs());
...
#ifdef PRISM_DEBUG_STATS
        register_stat(s_num_cycles_alert_buffer_nonempty).name("controller{}_abo_dbg_cycles_alert_nonempty", m_channel_id);
        register_stat(s_num_cycles_alert_buffer_nonempty_before_recovery).name("controller{}_abo_dbg_cycles_alert_nonempty_before_recovery", m_channel_id);
        register_stat(s_num_demand_rejected_due_to_fits).name("controller{}_abo_dbg_demand_rejected_fits", m_channel_id);
        register_stat(s_num_active_rejected_due_to_fits).name("controller{}_abo_dbg_active_rejected_fits", m_channel_id);
        register_stat(s_num_demand_deferred_rank_blocked).name("controller{}_abo_dbg_demand_deferred_rank_blocked", m_channel_id);
        register_stat(s_num_demand_deferred_pending_maint).name("controller{}_abo_dbg_demand_deferred_pending_maint", m_channel_id);

        register_stat(s_num_cycles_alert_buffer_head_prea).name("controller{}_abo_dbg_cycles_head_prea", m_channel_id);
        register_stat(s_num_cycles_alert_buffer_head_rfmab).name("controller{}_abo_dbg_cycles_head_rfmab", m_channel_id);
        register_stat(s_num_cycles_alert_buffer_head_not_ready).name("controller{}_abo_dbg_cycles_head_not_ready", m_channel_id);
        register_stat(s_num_demand_issued_while_alert_buffer_nonempty).name("controller{}_abo_dbg_demand_issued_while_alert_nonempty", m_channel_id);
        register_stat(s_num_active_issued_while_alert_buffer_nonempty).name("controller{}_abo_dbg_active_issued_while_alert_nonempty", m_channel_id);
        register_stat(s_num_cycles_alert_buffer_head_other).name("controller{}_abo_dbg_cycles_head_other", m_channel_id);
        register_stat(s_num_cycles_alert_buffer_rfmab_blocked_by_pre_recovery).name("controller{}_abo_dbg_cycles_rfmab_blocked_pre_recovery", m_channel_id);
        register_stat(s_num_priority_issued_while_alert_buffer_nonempty).name("controller{}_abo_dbg_priority_issued_while_alert_nonempty", m_channel_id);
    ...
    // True iff some item in the alert buffer targets `rank`. Used by the
    // rank-disjoint demand path: a rank with no pending alert items can
    // keep serving demand during the rest of the alert window.
    bool is_rank_blocked_by_alert(int rank) {
        if (rank < 0) return false;
        for (auto it = m_alert_buffer.begin(); it != m_alert_buffer.end(); ++it) {
    ...
    void tick() override {
        ...
        m_cur_next_recovery_clk = m_prism->next_recovery_cycle();
        m_cur_prism_state       = m_prism->get_state();
        ...
        bool is_recovery_setup    = m_alert_buffer.size() != 0;
        ...
        if (is_recovery_starting && !is_recovery_setup) {
            ...
            for (int i = 0; i < m_prism->get_num_abo_recovery_refs(); i++) {
                for (int j = 0; j < m_dram->get_level_size("rank"); j++) {
                    m_rfmab_template->addr_vec[m_rank_addr_idx] = j;
                    m_alert_buffer.enqueue(*m_rfmab_template);
                }
            }
        }

#ifdef PRISM_DEBUG_STATS
        // Per-cycle snapshot of the alert buffer's state.
        if (m_alert_buffer.size() != 0) {
            s_num_cycles_alert_buffer_nonempty++;

            bool before_recovery =
                m_cur_next_recovery_clk != std::numeric_limits<Clk_t>::max() &&
                m_clk < m_cur_next_recovery_clk;
            if (before_recovery) s_num_cycles_alert_buffer_nonempty_before_recovery++;

            auto head_it   = m_alert_buffer.begin();
            ...
            if      (head_cmd == cmd_prea)  s_num_cycles_alert_buffer_head_prea++;
            else if (head_cmd == cmd_rfmab) s_num_cycles_alert_buffer_head_rfmab++;
            else                            s_num_cycles_alert_buffer_head_other++;

            if (!m_dram->check_ready(head_cmd, head_it->addr_vec)) {
                s_num_cycles_alert_buffer_head_not_ready++;
            }
            if (head_cmd == cmd_rfmab && m_cur_prism_state == IPRISM::ABOState::PRE_RECOVERY) {
                s_num_cycles_alert_buffer_rfmab_blocked_by_pre_recovery++;
            }
        }
#endif

        bool alert_buffer_was_nonempty = (m_alert_buffer.size() != 0);
...
#ifdef PRISM_DEBUG_STATS
        if (request_found && alert_buffer_was_nonempty && buffer != &m_alert_buffer) {
            if      (buffer == &m_active_buffer)                            s_num_active_issued_while_alert_buffer_nonempty++;
            else if (buffer == &m_read_buffer || buffer == &m_write_buffer) s_num_demand_issued_while_alert_buffer_nonempty++;
            else if (buffer == &m_priority_buffer)                          s_num_priority_issued_while_alert_buffer_nonempty++;
        }
#endif
...
private:
    ...
    bool schedule_request(ReqBuffer::iterator& req_it, ReqBuffer*& req_buffer) {
        ...
        if (req_it = m_scheduler->get_best_request(m_active_buffer); req_it != m_active_buffer.end()) {
            bool fits  = m_clk + m_prism->min_cycles_with_preall(req_it) < next_recovery_clk;
        ...
        if (!request_found) {
            // 2.2.1  Alert (PrISM ABO recovery).
            //        [PrISM] Block RFMab while PRE_RECOVERY — tABO_ACT must elapse before any RFMab may issue.
            if (m_alert_buffer.size() != 0) {
                req_buffer = &m_alert_buffer;
                req_it     = m_alert_buffer.begin();
                req_it->command = m_dram->get_preq_command(req_it->final_command, req_it->addr_vec);

                bool is_rfm      = req_it->command == m_dram->m_commands("RFMab");
                bool is_pre_rec  = m_cur_prism_state == IPRISM::ABOState::PRE_RECOVERY;
                bool early_issue = is_rfm && is_pre_rec;
                request_found = !early_issue && m_dram->check_ready(req_it->command, req_it->addr_vec);
            }

            // 2.2.2  Priority buffer (REF / RFMsb maintenance).
            //        REF on the SAME rank as the alert head defers the alert
            //        head; REF on a DIFFERENT rank does not.
            //
            //        Per JEDEC ABO protocol, only REFab/REFsb are permitted during recovery
            ...
            //              allowed, even while the alert buffer is nonempty.
